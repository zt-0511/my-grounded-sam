...
# === 修改后的函数：增加了 max_area_threshold 参数，并清理了重复代码 ===
...
if __name__ == "__main__":
    ...
    # 默认值给0.30，防止配置文件没写报错
    max_area_thresh = config.get("max_area_threshold", 0.30)
    ...
    print(f"🚀 正在使用提示词进行检测: '{text_prompt}' (Box阈值: {box_thresh}, 面积阈值: {max_area_thresh})")
    ...
    boxes_filt, pred_phrases = get_grounding_output(
        ...
        box_thresh, 
        text_thresh, 
        max_area_threshold=max_area_thresh, # <--- 关键修改点
        device=device
    )
